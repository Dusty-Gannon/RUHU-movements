...
```{r}
  ...
  load("~/Documents/Columbine/Analysis/Bird_movements/Bilinear_dyad_models/fit_sender_receiver.RData")

#reader locations
  rlocs <- read.table("~/Documents/Columbine/Data/Bird_data/hja_reader_locations_centercomb.txt", header = T, sep = "\t")
  
# observed movements
  mvmnts <- read.table("~/Documents/Columbine/Data/Bird_data/total_movements_by_year.tsv",
...
# colors for plot
 forest.col1 <- "grey" 
 forest.col2 <- rgb(129,178,154, maxColorValue = 255)
 highlight <- rgb(242,204,143, maxColorValue = 255)
 nonforest.col <- rgb(244,241,222, maxColorValue = 255)
 rdr.col <- rgb(224,122,95, maxColorValue = 255)
 edge.col <- "black" #rgb(61,64,91, maxColorValue = 255)
...
# Create figure
 ...
 graph_plot <- ggplot()+
   ...
   geom_segment(data = mvs_all_df, 
                ...
                lineend = "round", size=mvs_all_df$obs/max(mvs_all_df$obs)*5,
                color=edge.col)+
   geom_point(data = rlocs, aes(x=x,y=y), size=3)+
   geom_point(data = rlocs, aes(x=x,y=y), color=rdr.col, size=2.5)+
   geom_segment(aes(x=570600, xend=571600, y=4902000, yend=4902000), 
                size=2.5)+
   annotate("text", x=570600, y=4901800, 
            label="1 Km", hjust=0)+
